...
class AnketaMarkdownParser:
    ...
    def _parse_company_info(self, content: str) -> dict:
        ...
        patterns = {
            'company_name': r'\| Компания \| (.+?) \|',
            'industry': r'\| Отрасль \| (.+?) \|',
            'specialization': r'\| Специализация \| (.+?) \|',
            'website': r'\| Сайт \| (.+?) \|',
            'contact_name': r'\| Контактное лицо \| (.+?) \|',
            'contact_role': r'\| Должность \| (.+?) \|',
        }
        ...
        # Parse services list
        result['services'] = self._parse_list_section(content, 'Услуги / Продукты')
        result['client_types'] = self._parse_list_section(content, 'Типы клиентов')
    ...
    def _parse_business_context(self, content: str) -> dict:
        ...
        return {
            'current_problems': self._parse_list_section(content, 'Текущие проблемы'),
            'business_goals': self._parse_list_section(content, 'Цели автоматизации'),
            'constraints': self._parse_list_section(content, 'Ограничения'),
        }
    ...
    def _parse_voice_agent(self, content: str) -> dict:
        ...
        # Extract table values
        patterns = {
            'agent_name': r'\| Имя агента \| (.+?) \|',
            'agent_purpose': r'\| Назначение \| (.+?) \|',
            'language': r'\| Язык \| (.+?) \|',
        }
        ...
        voice_match = re.search(r'\| Голос \| (.+?) \|', content)
        if voice_match:
        ...
        direction_match = re.search(r'\| Тип звонков \| (.+?) \|', content)
        if direction_match:
            direction_str = direction_match.group(1).strip()
            direction_map = {
                'Входящие': 'inbound',
                'Исходящие': 'outbound',
                'Входящие и исходящие': 'both',
            }
    ...
    def _parse_agent_functions(self, content: str) -> dict:
        ...
        # Find "Все функции агента" section
        ...
        priority_pattern = r'\*Приоритет: (\w+)\*'
    ...
    def _parse_integrations(self, content: str) -> dict:
        ...
        if 'не требуются' in section_content.lower():
            return {'integrations': integrations}

        # Parse table rows
        row_pattern = r'\| (.+?) \| (.+?) \| (Да|Нет) \|'
        for match in re.finditer(row_pattern, section_content):
            name = match.group(1).strip()
            purpose = match.group(2).strip()
            required = match.group(3).strip() == 'Да'
    ...
    def _parse_faq_items(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 6\. FAQ с ответами')
        if not section or 'не сгенерирован' in section.lower():
    ...
    def _parse_objection_handlers(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 7\. Работа с возражениями')
        if not section or 'не проработаны' in section.lower():
    ...
    def _parse_sample_dialogue(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 8\. Пример диалога')
        if not section or 'не сгенерирован' in section.lower():
        ...
        for match in re.finditer(pattern, section, re.DOTALL):
            ...
            # Map role labels
            role = 'bot' if 'Агент' in role_str else 'client'
    ...
    def _parse_financial_metrics(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 9\. Финансовая модель')
        if not section or 'не указаны' in section.lower():
            return {'financial_metrics': metrics}

        # Parse table: | Метрика | Значение | Источник | Примечание |
        pattern = r'\| (.+?) \| (.+?) \| (.+?) \| (.+?) \|'
    ...
    def _parse_market_analysis(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 10\. Анализ рынка')
        if not section:
            return result
        ...
        if competitors_section and 'не определены' not in competitors_section.group(1).lower():
            ...
            for match in re.finditer(comp_pattern, comp_content, re.DOTALL):
                ...
                for line in details.split('\n'):
                    if '✅' in line:
                        strengths.append(line.split('✅')[-1].strip())
                    elif '❌' in line:
                        weaknesses.append(line.split('❌')[-1].strip())
        ...
        if insights_section and 'не сгенерированы' not in insights_section.group(1).lower():
            for line in insights_section.group(1).split('\n'):
                if line.startswith('- '):
                    ...
                    relevance = 'high' if '🔥' in line else ('medium' if '📊' in line else 'low')
        ...
        advantages_section = re.search(
            r'### Конкурентные преимущества клиента\n\n(.+?)(?=\n---|\n## |\Z)',
            section, re.DOTALL
        )
    ...
    def _parse_target_segments(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 11\. Целевые сегменты')
        if not section or 'не определены' in section.lower():
        ...
        for match in re.finditer(pattern, section, re.DOTALL):
            ...
            # Extract description (first paragraph before **Болевые точки**)
            desc_match = re.match(r'(.+?)(?=\n\n\*\*Болевые|\Z)', details, re.DOTALL)
            ...
            for line in details.split('\n'):
                if '😓' in line:
                    pain_points.append(line.split('😓')[-1].strip())
                elif '⚡' in line:
    ...
    def _parse_escalation_rules(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 12\. Правила эскалации')
        if not section or 'не определены' in section.lower():
        ...
        for match in re.finditer(pattern, section):
            ...
            if trigger in ('Триггер', '-------'):
                continue

            # Map urgency labels
            urgency = 'medium'
            if '🚨' in urgency_label or 'Немедленно' in urgency_label:
                urgency = 'immediate'
            elif '⏰' in urgency_label or 'час' in urgency_label:
                urgency = 'hour'
            elif '📅' in urgency_label or 'дня' in urgency_label:
                urgency = 'day'
    ...
    def _parse_success_kpis(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 13\. KPI и метрики успеха')
        if not section or 'не определены' in section.lower():
            return {'success_kpis': kpis}

        # Parse table: | KPI | Цель | Бенчмарк | Измерение |
        pattern = r'\| (.+?) \| (.+?) \| (.+?) \| (.+?) \|'
        for match in re.finditer(pattern, section):
            name = match.group(1).strip()
    ...
    def _parse_launch_checklist(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 14\. Чеклист запуска')
        if not section or 'не определён' in section.lower():
            return {'launch_checklist': checklist}

        # Pattern: - ☐/○ Item — Responsible **(обязательно)**
        for line in section.split('\n'):
            if not line.strip().startswith('- '):
                continue

            required = '☐' in line or '**(обязательно)**' in line
            text = line[2:].strip()
            ...
            if '👤 Клиент' in text:
                ...
                responsible = 'team'
            elif '🤝 Совместно' in text:
                responsible = 'both'

            # Clean item text
            item_text = re.sub(r'\s*—\s*(👤 Клиент|👥 Команда|🤝 Совместно).*', '', text).strip()
    ...
    def _parse_ai_recommendations(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 15\. Рекомендации AI-эксперта')
        if not section or 'не сгенерированы' in section.lower():
            return {'ai_recommendations': recommendations}

        # Pattern: ### N. Priority Recommendation
        pattern = r'### \d+\. (?:🔴|🟡|🟢)?\s*(.+?)\n\n\*\*Ожидаемый эффект:\*\* (.+?)\n\n\*Приоритет: (\w+) \| Затраты: (\w+)\*'
        for match in re.finditer(pattern, section, re.DOTALL):
            ...
            # Map effort labels
            effort_map = {'Низкие': 'low', 'Средние': 'medium', 'Высокие': 'high'}
    ...
    def _parse_tone_of_voice(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 16\. Тон коммуникации')
        if not section or 'не определён' in section.lower():
            return {'tone_of_voice': tone}

        # Parse Do section
        do_match = re.search(r'### ✅ Делать\n\n(.+?)(?=\n### |$)', section, re.DOTALL)
        if do_match:
            tone['do'] = do_match.group(1).strip()

        # Parse Don't section
        dont_match = re.search(r'### ❌ Не делать\n\n(.+?)(?=\n### |\n---|\Z)', section, re.DOTALL)
        if dont_match:
    ...
    def _parse_error_handling_scripts(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 17\. Скрипты обработки ошибок')
        if not section or 'не определены' in section.lower():
            return {'error_handling_scripts': scripts}

        # Parse each script block
        label_map = {
            '🤔 Не понял запрос': 'not_understood',
            '⚠️ Техническая проблема': 'technical_issue',
            '🚫 Вне компетенции': 'out_of_scope'
        }
    ...
    def _parse_follow_up_sequence(self, content: str) -> dict:
        ...
        section = self._extract_section(content, r'## 18\. Последовательность follow-up')
        if not section:
