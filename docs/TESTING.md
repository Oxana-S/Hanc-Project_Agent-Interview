...
- [Этап 1: Юнит-тесты](#этап-1-юнит-тесты)
  - [1.7 Mid-session Voice Config](#17-mid-session-voice-config-тесты-v50--v53)
  - [1.8 LLM Мульти-провайдер тесты](#18-llm-мульти-провайдер-тесты-v52)
  - [1.9 v4.5-v4.6 Anketa Extraction & UI](#19-v45-v46-anketa-extraction--ui-тесты)
...
- [Ручное тестирование v5.0](#ручное-тестирование-v50--чеклист-для-пользователя)
  - [16. v4.4 — Microphone Reconnect](#16-v44--microphone-reconnect-возобновление-паузированной-сессии)
  - [17. v4.5-v4.6 — Anketa Extraction & UI](#17-v45-v46--anketa-extraction--ui-improvements)
...
```bash
# 1. Юнит-тесты + интеграционные (Этап 1 + 2.6 — должны пройти все, 1817 тестов)
...
# 8. v4.4-v4.6 Manual QA (10 минут — ручное тестирование критичных фич)
# a) v4.4 — Microphone Reconnect
# Создать сессию → пауза → выйти → вернуться по ссылке → resume → микрофон работает
# Expected: статус 'paused' в БД, микрофон активируется после resume

# b) v4.5 — Phone/Email Extraction
# Продиктовать: "плюс 43 664 755 03580" и "channel эт gmail точка com"
# Expected: phone и email в анкете

# c) v4.6 — Contextual Understanding
# Агент: "Какие задачи: A, B, C?" → Ответ: "Да все"
# Expected: agent_functions содержит все 3 опции

# d) Debug Mode
# Открыть http://localhost:8000?debug=true → debug panel виден в левом нижнем углу
# Expected: panel с completion, fields, messages, polling status

# См. детальные чек-листы в секциях 16-17 "Ручное тестирование v5.0"
```
...
|---------|---------|------------------|
| Тесты passed | 100% | 1815/1815 (+ 24 TODO для v4.5-v4.6) |
| Coverage | ≥50% | 50% |
| Критические модули | ≥80% | см. таблицу ниже |

> **⚠️ TODO:** Добавить 24 автоматических теста для v4.5-v4.6 (phone/email extraction, contextual understanding, debug mode). См. [секцию 1.9](#19-v45-v46-anketa-extraction--ui-тесты).
...
│   ├── test_consultant_core.py        # Voice consultant core functions (120 тестов)
...
│   │   ├── TestApplyVoiceConfigUpdate       # Mid-session speed/silence + voice lock (17 тестов)
...
**Итого: 39 тестовых файлов, 1817 тестов** (1790 unit + 27 integration).
...
### 1.7 Mid-session Voice Config тесты (v5.0 → v5.3)

Тесты покрывают применение настроек mid-session. **Важно (v5.3):** `voice_gender` НЕ меняется mid-session (Azure API locks voice после первого аудио). Speed и silence отправляются **изолированными** `update_options()` вызовами (сбой одного не блокирует другой). UI блокирует voice/type/llm_provider при активной сессии; speed/silence/verbosity доступны через Dashboard и Quick Settings panel (⚙).
...
#### `TestApplyVoiceConfigUpdate` (17 тестов)
...
| 6 | `test_voice_change_skipped_mid_session` | neutral→male → `update_options` НЕ вызывается (Azure locks voice) |
| 7 | `test_multiple_changes_isolated_calls` | speed+silence → 2 изолированных `update_options` (не 1 combined) |
...
| 15 | `test_all_five_settings_changed` | speed+silence → 2 isolated calls + verbosity async task (voice skipped) |
| 16 | `test_speed_failure_does_not_block_silence` | speed RuntimeError → silence всё равно вызывается (изоляция) |
| 17 | `test_voice_change_logs_skip_message` | voice change → log.info с "skipped" / "locks voice" |
...
### 1.9 v4.5-v4.6 Anketa Extraction & UI тесты

> **Статус:** ⚠️ TODO — автоматические тесты для новых фич ещё не написаны

Фичи v4.5-v4.6 требуют следующих тестов:

#### `test_data_cleaner.py` — `TestPhoneEmailPatterns` (TODO: 8 тестов)

| # | Тест | Что проверяет |
|---|------|---------------|
| 1 | `test_extract_phone_standard_format` | "+43 664 755 03580" → правильно извлекается |
| 2 | `test_extract_phone_speech_to_text` | "плюс сорок три..." → "+43664755..." |
| 3 | `test_extract_phone_with_pauses` | "плюс... семь... девятьсот" → корректная склейка |
| 4 | `test_extract_email_standard` | "channel.my.hani@gmail.com" → правильно извлекается |
| 5 | `test_extract_email_speech_to_text_et` | "channel эт gmail точка com" → "channel@gmail.com" |
| 6 | `test_extract_email_speech_to_text_sobaka` | "info собака компания точка ru" → "info@компания.ru" |
| 7 | `test_validate_phone_length` | Телефон с 5 цифрами → rejected (7-15 required) |
| 8 | `test_validate_email_no_at` | Email без @ → rejected |

#### `test_anketa_extractor.py` — `TestContextualPostProcessing` (TODO: 12 тестов)

| # | Тест | Что проверяет |
|---|------|---------------|
| 1 | `test_contextual_agent_functions_da_vse` | А: "администратор, напоминатель?" К: "да все" → все опции |
| 2 | `test_contextual_agent_functions_vsio_podhodit` | "всё подходит" → все опции |
| 3 | `test_contextual_agent_functions_da_odnoznachno` | "да, однозначно" → все опции |
| 4 | `test_contextual_integrations_da` | А: "CRM интеграция?" К: "да" → integrations filled |
| 5 | `test_contextual_partial_answer` | "только первые два" → первые 2 опции (не все) |
| 6 | `test_no_contextual_when_explicit` | Явное перечисление задач → не триггерит context logic |
| 7 | `test_affirmative_patterns` | Все паттерны: "конечно", "именно так", "подходит" |
| 8 | `test_fallback_phone_extraction` | LLM не извлёк → fallback regex находит |
| 9 | `test_fallback_email_extraction` | LLM не извлёк → fallback regex находит |
| 10 | `test_fallback_preserves_llm_data` | Fallback не перезаписывает существующие данные |
| 11 | `test_post_processing_order` | Contextual → Fallback → Cleanup (правильный порядок) |
| 12 | `test_empty_dialogue_no_crash` | Пустой диалог → graceful, нет crash |

#### `test_api_server.py` — `TestDebugMode` (TODO: 4 теста)

| # | Тест | Что проверяет |
|---|------|---------------|
| 1 | `test_anketa_polling_includes_completion` | `GET /api/session/{id}/anketa` → completion_rate в ответе |
| 2 | `test_anketa_polling_includes_field_count` | Ответ содержит fields_filled (count) |
| 3 | `test_anketa_polling_error_handling` | Несуществующая сессия → 404 (не 500) |
| 4 | `test_anketa_polling_rate_limit` | 10 запросов в секунду → не падает |

#### Frontend тесты (manual / E2E)

> **Статус:** ⚠️ Только ручное тестирование (секция 17 в "Ручное тестирование v5.0")

Frontend фичи (debug panel, header indicator, improved progress bar) покрыты только ручными тестами из-за сложности автоматизации LiveKit + browser UI.

**Рекомендация:** Добавить Playwright/Cypress E2E тесты для:
- Debug panel appearance при `?debug=true`
- Header indicator "⚡ Анкета заполняется..." visibility
- Progress bar анимация при обновлении
- Toast notifications длительность и цвет

#### Покрытие по модулям (после v4.5-v4.6)

| Модуль | Coverage (до) | Coverage (после TODO) | Изменения |
|--------|---------------|----------------------|-----------|
| src/anketa/data_cleaner.py | 98% | **99%** | +1% (phone/email patterns) |
| src/anketa/extractor.py | 85% | **92%** | +7% (contextual + fallback) |
| public/app.js | N/A (frontend) | Manual QA | Debug panel, header indicator |

#### Запуск (после написания тестов)

```bash
# Phone/Email extraction
./venv/bin/python -m pytest tests/unit/test_data_cleaner.py::TestPhoneEmailPatterns -v

# Contextual post-processing
./venv/bin/python -m pytest tests/unit/test_anketa_extractor.py::TestContextualPostProcessing -v

# API debug mode
./venv/bin/python -m pytest tests/unit/test_api_server.py::TestDebugMode -v

# Все новые тесты
./venv/bin/python -m pytest tests/unit/ -k "PhoneEmail or Contextual or DebugMode" -v
```
...
[ ] Настройки голоса (speed, silence, verbosity) применяются mid-session; voice locked после первого аудио
...
[ ] Все 1817+ тестов по-прежнему проходят
...
[ ] БЕЗ активной сессии: все настройки доступны, нет баннера, нет badges
[ ] С активной сессией: баннер «🔒 Некоторые настройки заблокированы» виден
[ ] С активной сессией: Голос, Тип, LLM — серые с 🔒, некликабельные
[ ] С активной сессией: Скорость, Тишина, Ответы — badge «применяется сразу»
[ ] Клик по заблокированной настройке → toast с объяснением (2 сек)
```
...
[ ] Кнопка ⚙ (Quick Settings) видна рядом с 🎤 и ⏸
...
> **v5.3:** Голос, Тип, LLM модель **заблокированы** mid-session (Azure API locks voice). Speed/silence/verbosity меняются через Dashboard или Quick Settings (⚙).

```
[ ] На экране сессии: нажать ⚙ → Quick Settings panel появляется
[ ] Quick Settings: Скорость речи (slider), Задержка тишины (slider), Развёрнутость (сегменты)
[ ] Изменить скорость в Quick Settings на 1.5x → агент говорит быстрее
[ ] Изменить «Ответы» на «Кратко» → агент отвечает 1-2 предложениями
[ ] Клик вне Quick Settings panel → panel закрывается
[ ] Нажать «← Назад» → Dashboard
[ ] Баннер «🔒 Некоторые настройки заблокированы» виден
[ ] Голос, Тип, LLM модель — серые с 🔒, некликабельные
[ ] Скорость, Тишина, Ответы — доступны, badge «применяется сразу»
[ ] Изменить задержку тишины на 3.0с на Dashboard
[ ] Вернуться в сессию → Quick Settings синхронизированы с Dashboard
[ ] DevTools Network: PUT /api/session/{id}/voice-config → 200
[ ] DevTools Network: GET /api/session/{id}/reconnect → 200
[ ] Агент говорит БЫСТРЕЕ (скорость 1.5x)
[ ] Агент отвечает КРАТКО (без длинных вступлений)
...
```
[ ] На экране сессии: открыть Quick Settings → изменить «Ответы» на «Подробно»
[ ] Агент даёт РАЗВЁРНУТЫЕ ответы с примерами
[ ] Изменить скорость на 0.75x → агент говорит медленнее
[ ] Вернуться на Dashboard → скорость и ответы синхронизированы с Quick Settings
[ ] Завершить сессию → все настройки разблокированы, баннер исчез
```
...
[ ] Логи агента: grep "voice_config updated" /tmp/agent_entrypoint.log — есть записи mid-session update (speed/silence)
[ ] Логи агента: grep "skipped.*locks voice" /tmp/agent_entrypoint.log — voice НЕ менялся mid-session
...
### 16. v4.4 — Microphone Reconnect (возобновление паузированной сессии)

> **Контекст:** Исправлены 6 критичных багов при возврате к паузированной сессии (race condition, logger error, UI restore)

```
[ ] Создать новую консультацию (Экспертный режим)
[ ] Поговорить 2-3 минуты (10+ сообщений)
[ ] Нажать кнопку «Пауза» (⏸) → Pause overlay появляется
[ ] Нажать «Сохранить и выйти» → переход на Dashboard
[ ] Скопировать ссылку сессии (кнопка «Копировать ссылку»)
[ ] Открыть ссылку в новой вкладке (или вернуться через Dashboard)

EXPECTED (после возврата):
[ ] Pause overlay видно (полупрозрачный оверлей поверх чата)
[ ] Кнопка паузы показывает «▶» (play icon)
[ ] Микрофон DISABLED (кнопка неактивна, серая)
[ ] Connection status: «Подключено» (зелёный индикатор в header)
[ ] Voice status: «На паузе» (под progress bar)
[ ] Форма анкеты заполнена (данные сохранены из предыдущей сессии)
[ ] Dialogue history восстановлен (все сообщения на месте)
[ ] DevTools Console: НЕТ ошибки "Logger._log() got an unexpected keyword argument"
[ ] DevTools Console: НЕТ ошибки "Cannot read properties of null (reading 'publishTrack')"

ПРОДОЛЖЕНИЕ:
[ ] Нажать кнопку «▶» (Resume) → pause overlay исчезает
[ ] Микрофон активируется автоматически (кнопка зелёная)
[ ] Voice status: «Слушаю...» (или «Говорю...» если агент отвечает)
[ ] Можно продолжить диалог голосом
[ ] Агент слышит и отвечает (STT/TTS работают)
[ ] Анкета продолжает заполняться (progress bar обновляется)

DATABASE VERIFICATION:
[ ] sqlite3 data/sessions.db "SELECT session_id, status FROM sessions WHERE session_id='<SESSION_ID>';"
    → status = 'paused' (НЕ 'processing')
[ ] sqlite3 data/sessions.db "SELECT LENGTH(dialogue_history) FROM sessions WHERE session_id='<SESSION_ID>';"
    → > 1000 bytes (если было 10+ сообщений)

LOGS:
[ ] cat /tmp/agent_entrypoint.log | grep "dialogue_saved_sync" — есть записи с extra={}
[ ] cat /tmp/agent_entrypoint.log | grep "session_status_preserved" — статус 'paused' сохранён
```

### 17. v4.5-v4.6 — Anketa Extraction & UI Improvements

> **Контекст:** Полный план (6 часов): phone/email extraction, контекстное понимание LLM, проактивные уведомления, улучшенная видимость UI, debug mode

#### 17.1. SPRINT 1 — Phone/Email Extraction (speech-to-text)

```
[ ] Создать новую консультацию
[ ] Агент спросит: «Как с вами связаться?»
[ ] Продиктовать телефон голосом:
    «Позвоните мне по номеру плюс сорок три шесть шесть четыре семь пять пять ноль три пять восемь ноль»
[ ] Продиктовать email голосом:
    «Мой емейл ченнел точка май точка хани эт джи мейл точка ком»
[ ] Подождать 10-15 секунд (следующий цикл extraction)

EXPECTED:
[ ] В анкете справа появилось поле «Телефон»: "+43 664 755 03580"
[ ] В анкете справа появилось поле «Email»: "channel.my.hani@gmail.com"
[ ] Progress bar обновился (completion_rate увеличился на ~10-15%)
[ ] Toast уведомление: «Анкета обновлена — +2 поля» (зелёный, 4s)

EDGE CASES:
[ ] Продиктовать email с "собака" вместо "эт": «info собака компания точка ru»
    → email: "info@компания.ru"
[ ] Продиктовать телефон с паузами: «плюс... семь... девятьсот один... два три четыре»
    → phone корректно собирается (без точек)

DATABASE:
[ ] sqlite3 data/sessions.db "SELECT contact_phone, contact_email FROM sessions WHERE session_id='<SESSION_ID>';"
    → contact_phone и contact_email заполнены
```

#### 17.2. SPRINT 2 — Contextual Understanding (контекстное понимание "да/нет")

```
[ ] Создать новую консультацию
[ ] Агент спросит: «Какие задачи хотите автоматизировать: администратор, напоминатель, консультант, маршрутизатор?»
[ ] Ответить голосом: «Да все интересны»
[ ] Подождать 10-15 секунд (extraction cycle)

EXPECTED:
[ ] В анкете справа появилось поле «Задачи агента»
[ ] В списке задач ВСЕ 4 опции:
    - Администратор
    - Напоминатель
    - Консультант
    - Маршрутизатор
[ ] Progress bar обновился (+15-20%)
[ ] Toast: «Анкета обновлена — +1 поле»

VARIATIONS (протестировать разные утвердительные фразы):
[ ] «Всё подходит» → все опции заполнены
[ ] «Да, однозначно» (на вопрос про интеграцию с CRM) → integrations заполнены
[ ] «Конечно, это важно» → соответствующее поле заполнено

NEGATIVE CASE:
[ ] Агент: «Какие задачи: A, B, C?»
[ ] Ответ: «Только первые два»
[ ] EXPECTED: agent_tasks содержит только A и B (не все)

DATABASE:
[ ] sqlite3 data/sessions.db "SELECT json_extract(anketa_data, '$.agent_functions') FROM sessions WHERE session_id='<SESSION_ID>';"
    → JSON массив с 4 элементами
```

#### 17.3. SPRINT 3 — Proactive Notifications (проактивное уведомление)

```
[ ] Создать новую консультацию
[ ] Через 2 секунды после подключения (RoomEvent.Connected):

EXPECTED:
[ ] Toast появляется: «💡 Агент автоматически заполняет анкету во время разговора» (6s, синий)
[ ] В header появляется индикатор: «⚡ Анкета заполняется... 0%»
[ ] Header indicator обновляется каждые 2s (синхронно с polling)
[ ] После первого заполнения: «⚡ Анкета заполняется... 15%»
[ ] После достижения 100%: header indicator скрывается через 5s

BEFORE (для сравнения):
- Пользователь НЕ знал что анкета заполняется до первого toast (после 6 сообщений)

AFTER:
- Пользователь видит "Анкета заполняется..." СРАЗУ после подключения
```

#### 17.4. SPRINT 4 — UI Visibility (улучшенная видимость индикаторов)

```
[ ] Создать новую консультацию
[ ] Поговорить 3-4 минуты (набрать ~30% completion)

VISUAL INSPECTION:
[ ] Progress bar: высота 10px (БЫЛО: 6px) — заметно толще
[ ] Progress bar: box-shadow видно (inset для глубины)
[ ] Progress bar: при обновлении — анимация свечения (progressPulse)
[ ] Progress text: font-size 0.9rem, цвет яркий (text-primary), жирный (font-weight: 600)
[ ] Status ticker dot: размер 8px (БЫЛО: 6px), pulse анимация с расширением
[ ] Status ticker text: font-size 0.9rem, цвет яркий, font-weight: 500
[ ] Toast при обновлении: зелёный цвет ('success', НЕ 'info'), длительность 4s (БЫЛО: 2.5s)

ANIMATION CHECK:
[ ] Изменить значение поля в анкете → progress bar обновляется
[ ] В момент обновления: зелёное свечение вокруг progress fill (box-shadow pulse)
[ ] Status ticker dot: пульсирует с кольцом расширения (не просто opacity)
```

#### 17.5. SPRINT 5 — Debug Mode (режим отладки)

```
[ ] Добавить в URL: ?debug=true (или в DevTools Console: localStorage.setItem('anketa_debug', 'true'))
[ ] Перезагрузить страницу

EXPECTED:
[ ] В левом нижнем углу появляется debug panel (чёрный фон, белый текст, монопространственный шрифт)
[ ] Debug panel содержит:
    ⚙️ Anketa Debug
    Completion: 0%
    Fields: 0/28
    Messages: 0
...
[ ] Создать консультацию, поговорить 2-3 минуты
[ ] Debug panel обновляется каждые 2s:
    - Completion: 35%
    - Fields: 8/28
    - Messages: 14
    - Polling: active (зелёный)
    - Errors: 0

[ ] Нажать кнопку «Close» → panel исчезает
[ ] localStorage.getItem('anketa_debug') === null (debug mode отключен)

LOGS:
[ ] DevTools Console: [POLLING] Anketa update {session_id, completion_rate, fields_filled, ...}
[ ] DevTools Console: [DEBUG] Debug panel initialized (при включении debug mode)
[ ] cat logs/anketa.log | grep "extraction_result" — enhanced logging с fields_filled, has_phone, has_email

ERROR SIMULATION:
[ ] Остановить сервер: make stop
[ ] Debug panel: Polling: stopped (красный)
[ ] Debug panel: Errors: 3 (после 3 неудачных попыток)
[ ] Toast: «Проблема с обновлением анкеты. Попробуйте обновить страницу.» (warning, 5s)
```

#### 17.6. Integration Test (все спринты вместе)

```
[ ] Открыть http://localhost:8000?debug=true
[ ] Создать консультацию

TIMELINE:
T+2s:  Toast «💡 Агент заполняет анкету» + Header indicator появляется
T+30s: Продиктовать телефон «плюс 43 664...» и email «channel эт gmail точка com»
T+45s: Агент: «Какие задачи: администратор, напоминатель?» → «Да все»
T+60s: Progress bar: 40%, debug panel: Fields: 10/28, Toast: «+3 поля» (зелёный, 4s)

EXPECTED STATE (T+60s):
[ ] Header: «⚡ Анкета заполняется... 40%»
[ ] Progress bar: 10px, свечение, анимация при обновлении
[ ] Status ticker: «Заполнено 10 из 28 полей», dot пульсирует
[ ] Debug panel: Completion: 40%, Fields: 10/28, Messages: 12, Errors: 0
[ ] Анкета справа: phone, email, agent_tasks заполнены
[ ] DevTools Console: [POLLING] логи с полными метриками

VERIFICATION:
[ ] sqlite3 data/sessions.db "SELECT completion_rate, contact_phone, contact_email FROM sessions WHERE session_id='<SESSION_ID>';"
    → completion_rate > 0.4, phone и email заполнены
```
...
| 2. Settings Panel | 11 | /11 |
| 3. Создание сессии | 13 | /13 |
| 4. Пауза и resume | 4 | /4 |
| 5. Смена настроек mid-session | 16 | /16 |
...
| 16. v4.4 Microphone Reconnect | 23 | /23 |
| 17.1. Phone/Email Extraction | 10 | /10 |
| 17.2. Contextual Understanding | 10 | /10 |
| 17.3. Proactive Notifications | 6 | /6 |
| 17.4. UI Visibility | 9 | /9 |
| 17.5. Debug Mode | 14 | /14 |
| 17.6. Integration Test | 11 | /11 |
| **ИТОГО** | **217** | **/217** |
